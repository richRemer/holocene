...
function Database(holo, name, dbPath) {
    ...
    /**
     * Lock the database.  If a problem occurs, an error will be passed to
     * the callback.
     * @param {function} done
     */
    ...
    /**
     * Unlock the database.  If a problem occurs, an error will be passed
     * to the callback.
     * @param {function} done
     */
    ...
    this.drop = function(done) {
        ...
        if (!this.locked) return done(new Error("db not locked"));
